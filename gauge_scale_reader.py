...
class GaugeScaleReader:
    ...
    def read_scale(self, image_or_path, visualize=True, score_thresh=0.5, cap_value=100000):
        ...
        try:
            raw, _ = self.ocr_engine(tmp_path)
        ...
        for item in results:
            ...
            if m:
                val = float(text)
                if val <= cap_value:   # 🚀 quick cap fix
                    num_items.append((val, pts, conf))
            else:
...
if __name__ == "__main__":
    # Example usage
    img_path = "C:\\Users\\bevan\\OneDrive\\Desktop\\DEV_STUFF\\VS FILES\\guage_reader\\gauge-yolov11\\Screenshot 2025-06-09 120205.png"  # replace with your gauge image path
